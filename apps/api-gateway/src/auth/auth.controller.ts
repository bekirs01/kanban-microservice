...
export class AuthController {
  ...
  @ApiResponse({
    status: 200,
    description:
      "Login realizado com sucesso. Retorna tokens (access_token, refresh_token).",
  })
  @ApiResponse({
    status: 401,
  ...
  @Post("/register")
  @ApiOperation({ summary: "Registrar novo usuário (direct; prefer register-request)" })
  @ApiResponse({
    status: 201,
    description:
      "Usuário registrado com sucesso. Retorna tokens (access_token, refresh_token, user).",
  })
  @ApiResponse({
    status: 400,
  ...
  @ApiOperation({
    summary: "Submit signup request for ADMIN approval",
  })
  @ApiResponse({ status: 201 })
  @ApiBody({ type: SubmitRegistrationRequestDto })
  submitRegistrationRequest(@Body() dto: SubmitRegistrationRequestDto) {
    return this.authClient.send("auth.registration.submit", dto);
  }
  ...
  @ApiResponse({
    status: 200,
    description: "Token renovado com sucesso. Retorna novo access_token.",
  })
  ...
  @ApiResponse({
    status: 200,
    description:
      "Logout realizado com sucesso. Invalida o refresh token do usuário.",
  })
